...
export default function V0ParticleAnimation() {
  ...
  // Auto-trigger scatter effect on page load
  useEffect(() => {
    // 1. Trigger the effect immediately on load
    setActiveEffects({ scatter: true })

    // 2. Start a timer for 3 seconds (3000ms)
    const timer = setTimeout(() => {
      // 3. Turn the effect off (particles return to original spot)
      setActiveEffects({})
    }, 3000)

    // Cleanup: specifically good practice in React to prevent bugs if the user leaves the page early
  ...
  // Distance to rounded box (for '0')
  const roundedBox = (px: number, py: number, cx: number, cy: number, w: number, h: number, r: number) => {
  ...
  // Distance to bl0ck text shape
  const dist = (px: number, py: number) => {
    const w = 0.045 // stroke width - consistent across all letters
    const boxW = 0.15 // box width for rounded box letters
    const boxH = 0.25 // box height for rounded box letters
    const boxR = 0.06 // corner radius for rounded boxes
    const innerW = 0.08 // inner box width
    const innerH = 0.18 // inner box height
    const innerR = 0.04 // inner corner radius

    // Manual kerning - optical spacing based on visual weight of each letter
    const gap_B_L = 0.45   // Tight - L is thin, needs to nestle close to B
    const gap_L_0 = 0.50   // Wider - prevent L from blending into 0
    const gap_0_C = 0.50   // Standard - both wide shapes need full space
    const gap_C_K = 0.30   // Very tight - C's opening provides significant visual space

    // Calculate letter positions sequentially using optical gaps
    // Center the '0' at x=0 (middle letter)
    const bX = -0.95                // Starting point (adjusted to center '0')
    const lX = bX + gap_B_L         // -0.50
    const zeroX = lX + gap_L_0      // 0.00 (centered!)
    const cX = zeroX + gap_0_C      // 0.50
    const kX = cX + gap_C_K         // 0.80

    // Letter 'B' - vertical stem with two rounded rectangular bowls
    const bStem = g(px, py, bX, -boxH, bX, boxH, w)
    // Upper bowl - rounded box
    const bUpperOuter = roundedBox(px, py, bX + 0.13, boxH/2, boxW * 0.6, boxH * 0.4, boxR * 0.8)
    const bUpperInner = roundedBox(px, py, bX + 0.13, boxH/2, innerW * 0.6, innerH * 0.4, innerR * 0.8)
    const bUpperRing = Math.max(bUpperOuter, -bUpperInner)
    const bUpperCut = Math.max(bUpperRing, -(px - bX))
    // Lower bowl - rounded box
    const bLowerOuter = roundedBox(px, py, bX + 0.13, -boxH/2, boxW * 0.6, boxH * 0.4, boxR * 0.8)
    const bLowerInner = roundedBox(px, py, bX + 0.13, -boxH/2, innerW * 0.6, innerH * 0.4, innerR * 0.8)
    const bLowerRing = Math.max(bLowerOuter, -bLowerInner)
    const bLowerCut = Math.max(bLowerRing, -(px - bX))
    const bLetter = Math.min(bStem, Math.min(bUpperCut, bLowerCut))

    // Letter 'L' - vertical stem with horizontal base (using capsules for consistency)
    const lVertical = g(px, py, lX, -boxH, lX, boxH, w)
    const lHorizontal = g(px, py, lX, -boxH, lX + boxW + 0.05, -boxH, w)
    const lLetter = Math.min(lVertical, lHorizontal)

    // Digit '0' - rounded box with diagonal slash (like original v0 logo)
    const zeroOuter = roundedBox(px, py, zeroX, 0, boxW, boxH, boxR)
    const zeroInner = roundedBox(px, py, zeroX, 0, innerW, innerH, innerR)
    const zeroRing = Math.max(zeroOuter, -zeroInner)
    // Add diagonal slash from bottom-left to top-right
    const zeroSlash = g(px, py, zeroX - boxW * 0.8, -boxH * 0.8, zeroX + boxW * 0.8, boxH * 0.8, w)
    const zeroLetter = Math.min(zeroRing, zeroSlash)

    // Letter 'C' - rounded box with right opening (like '0' but with gap)
    const cOuter = roundedBox(px, py, cX, 0, boxW, boxH, boxR)
    const cInner = roundedBox(px, py, cX, 0, innerW, innerH, innerR)
    const cRing = Math.max(cOuter, -cInner)
    // Cut right side for opening - create a wedge using angular cutout
    const cOpeningAngle = Math.atan2(py - 0, px - cX)
    const cInRange = cOpeningAngle > -Math.PI/2.5 && cOpeningAngle < Math.PI/2.5
    const cLetter = cInRange ? 1e5 : cRing

    // Letter 'K' - vertical stem with two diagonal arms
    const kStem = g(px, py, kX, -boxH, kX, boxH, w)
    // Upper arm from middle to top-right
    const kUpperArm = g(px, py, kX, 0.05, kX + boxW + 0.08, boxH, w)
    // Lower arm from middle to bottom-right
    const kLowerArm = g(px, py, kX, -0.05, kX + boxW + 0.08, -boxH, w)
    const kLetter = Math.min(kStem, Math.min(kUpperArm, kLowerArm))

    return Math.min(bLetter, Math.min(lLetter, Math.min(zeroLetter, Math.min(cLetter, kLetter))))
  }
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Generate particles
    const numParticles = 20000 // increased density for sharper edges
    const thickness = 0.1 // reduced depth to minimize blur
    const positions = new Float32Array(numParticles * 3)
    const colors = new Float32Array(numParticles * 3)
    let i = 0
    const maxAttempts = 2000000 // increased for higher particle count
    let attempts = 0
