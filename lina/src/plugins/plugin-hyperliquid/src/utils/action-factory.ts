...
export function createOpenPositionAction(config: OpenPositionConfig): Action {
  ...
  return {
    ...
    parameters: {
      ...
      leverage: {
        type: 'number',
        description: 'Leverage multiplier (1-25x, default: 1)',
        required: false,
      },
      orderType: {
        type: 'string',
    ...
    handler: async (
      ...
      try {
        ...
        if (!symbol) {
          throw new Error('Symbol is required (e.g., BTC, ETH, SOL)');
        }
    ...
    examples: side === 'long'
      ? [
          [
            { name: '{{user}}', content: { text: 'open a $1000 long on BTC with 5x leverage' } },
            { name: '{{agent}}', content: { text: 'Opening BTC long position with 5x leverage...', action: actionName } },
          ],
          [
            { name: '{{user}}', content: { text: 'long ETH $500' } },
            { name: '{{agent}}', content: { text: 'Opening ETH long position...', action: actionName } },
          ],
          [
            { name: '{{user}}', content: { text: 'go long SOL at 150 with limit order, $2000, 3x' } },
            { name: '{{agent}}', content: { text: 'Placing limit long order for SOL at $150...', action: actionName } },
          ],
        ]
      : [
          [
            { name: '{{user}}', content: { text: 'open a $1000 short on BTC with 5x leverage' } },
            { name: '{{agent}}', content: { text: 'Opening BTC short position with 5x leverage...', action: actionName } },
          ],
          [
          ...
          [
            { name: '{{user}}', content: { text: 'short SOL at 180 with limit order, $2000, 3x' } },
            { name: '{{agent}}', content: { text: 'Placing limit short order for SOL at $180...', action: actionName } },
          ],
        ],
  };
}
