    ...
    > ShardTree<S, DEPTH, SHARD_HEIGHT>
{
    ...
    fn root_internal(
        ...
        match &cap.root.0 {
            Node::Parent { ann, left, right } => {
                match ann {
                    Some(cached_root) if target_addr.contains(&cap.root_addr) && truncate_at >= cap.root_addr.position_range_end() => {
    ...
    fn witness_helper<Ctx>(
        ...
        get_shard(&ctx, Address::from_parts(Level::new(16), 752));
        get_shard(&ctx, Address::from_parts(Level::new(16), 751));
        get_shard(&ctx, Address::from_parts(Level::new(16), 750));
        get_shard(&ctx, Address::from_parts(Level::new(16), 749));

        println!("subtree_addr: {:?}", subtree_addr);
    ...
    pub fn witness_internal(
        &self,
        position: Position,
        as_of: Position,
    ) -> Result<MerklePath<H, DEPTH>, ShardTreeError<S::Error>> {
        let cap = self.store.get_cap();
        cap.map(|c| {
            fn go<H>(node: &Tree<Option<Arc<H>>, (H, RetentionFlags)>) -> usize {
                match &node.0 {
                    Node::Parent { ann, left, right } => {
                        go(&left) +
                        go(&right)
        ...
        Self::witness_helper(
            ...
            position,
            as_of,
            |ctx, shard_root| {let shard = ctx.store.get_shard(shard_root); println!("got shard for {shard_root:?}");
            // println!("shard: {:?}", shard);
            shard.as_ref().map(|s|s.as_ref().map(|s|{
